...
class Distillation_ImageDataset(Dataset):
    def __init__(
        ...
        if paired_transform is None and (s_transform is None or t_transform is None):
            raise ValueError(
                "Provide either 'paired_transform' or both 's_transform' and "
                "'t_transform'."
            )
    ...
    def __getitem__(self, index):
        ...
        if self.paired_transform is not None:
            s_img, t_img = self.paired_transform(img)
        else:
