...
class R1_mAP_eval():
    def __init__(self, max_rank=100, metric="cosine", reranking=False,
                 reranking_parameters=(60, 10, 0.3)):
        super(R1_mAP_eval, self).__init__()
        self.max_rank = max_rank
        self.metric = metric
        self.reranking = reranking
        self.reranking_parameters = reranking_parameters
        self.reset()
    ...
    def compute(self):  # called after each epoch
        ...
        if self.reranking:
            ...
            k1, k2, lambda_value = self.reranking_parameters
        ...
        cmc, all_AP, all_INP = evaluate_rank(
            distmat, q_pids, g_pids, q_camids, g_camids, max_rank=self.max_rank,
        )
        mAP = np.mean(all_AP)
