...
class Swin_Transformer_V2(nn.Module):
    ...
    def forward(self, x):
        ...
        # Local pooling feeds the same FC head as global pooling. Export the
        # final intermediate after the model's existing LayerNorm as well;
        # the returned final x (and global retrieval output) is unchanged.
        x, intermediates_feats = self.model.forward_intermediates(x, norm=True)
        avg = x.mean(dim=(1, 2))
