...
_C.EXPERIMENT = CN()
# Training uses CUDA; evaluation also supports CPU.
...
# True initializes the student from ImageNet or STUDENT_PRETRAIN_PATH.
# False trains from scratch. Evaluation loads its checkpoint directly.
...
# Resolution-Accessibility UGD.  Disabled by default so legacy UGD configs
# retain their original independent-view behavior.
_C.UGD.RA_ENABLED = False
# Keep saved scalar-RA experiments reproducible; new Food172 configs opt in.
_C.UGD.RA_MODE = "scalar"
# Directional local distance: 0 is aligned ordinary L2; must remain below 1.
_C.UGD.RA_DIRECTION_RHO = 0.25
# Legacy shared temperature.  V3 prefers the scale-specific values below;
# older YAML files can still override this value as a fallback.
_C.UGD.RA_TAU = 0.2
_C.UGD.RA_FINE_TAU = None
_C.UGD.RA_COARSE_TAU = None
_C.UGD.RA_FINE_FLOOR = 0.75
_C.UGD.RA_COARSE_FLOOR = 0.50
_C.UGD.RA_MIN_WEIGHT = 0.0
_C.UGD.RA_MAX_WEIGHT = 1.0
_C.UGD.RA_USE_CLASS_MASK = True
# Both the uniform base and RA modulation use the same aligned local branch.
_C.UGD.RA_BLEND = 0.25
# Normalize coefficient mass, not loss values or gradient norms.
_C.UGD.RA_BUDGET_NORMALIZE = True
# Match the antialiased downsampling used by the ordinary image resize.
_C.UGD.RA_ANTIALIAS = True
# Keep the established gate; "retrieval" is a separate target-space ablation.
_C.UGD.RA_ACCESSIBILITY_SPACE = "backbone"
# Frozen-teacher-classifier LSD. None inherits the old weight/temperature so
...
# Use one set of spatial augmentation parameters for the student/teacher KD
# views.  RA-UGD requires this to make local grid cells correspond.
_C.INPUT.KD_SYNC_AUGMENTATION = False
...
# Dataset root. Datasets must be downloaded/prepared separately.
...
_C.OUTPUT_DIR = CN()
_C.OUTPUT_DIR.ROOT_PATH = "./outputs"
